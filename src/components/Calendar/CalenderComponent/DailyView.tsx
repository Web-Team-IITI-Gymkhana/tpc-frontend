...
export default function DailyView({ date }: { date: any }) {
  ...
  useEffect(() => {
    const events = filteredEvents.filter(
      (evt: Event) =>
        dayjs(evt.startDateTime).format("DD-MM-YY") === date.format("DD-MM-YY"),
    );
    setDayEvents(events);
  }, [filteredEvents, date]);
  ...
  return (
    <div className="flex flex-col w-full space-y-4 ">
      <div className="flex flex-col items-start ml-12">
        <p className={`text-xs pl-1 text-gray-600 ${getCurrentDay()}`}>
          {date.format("ddd").toUpperCase()}
        </p>
        <div className="flex item-center justify-center">
          <p className={`text-2xl p-1 ${getCurrentDate()}`}>
            {date.format("DD")}
          </p>
        </div>
      </div>
      <span className="flex flex-row space-x-2">
        <div className="flex flex-col space-y-10 -mt-2">
          {time_list.map((time: string, i: number) => (
            <p key={i} className="text-xs text-gray-600 font-extralight">
              {time}
            </p>
          ))}
        </div>
