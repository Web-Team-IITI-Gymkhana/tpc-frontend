...
export default function SeasonModal({ open, setOpen, id,type,year }) {
  ...
  const fetchCurrentRegistrationData = async (studentId: any,type:any,year:any) => {
    setLoading(true);
    try {
      const data = await fetchRegistrationDataByIdAndSeason(studentId,type,year);
      setRegistrationDataCurrent(data);
      console.log(data);
    } catch (error) {
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    if (open && id) {
      ...
      fetchCurrentRegistrationData(id,type,year);
    }
  }, [open, id]);
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {registrationDataCurrent
                          ? registrationDataCurrent.map((registration) => (
